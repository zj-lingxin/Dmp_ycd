...
$SQOOP_HOME/bin/sqoop import --connect "jdbc:mysql://192.168.7.7:3306/asto_ec_origin?useUnicode=true&characterEncoding=utf-8&zeroDateTimeBehavior=convertToNull&transformedBitIsBoolean=true" --username root --password password --query 'SELECT s.shop_src_id,y.order_id,y.order_date,y.cigar_name,y.wholesale_price,y.purchase_amount,y.order_amount,y.money_amount,y.producer_name,t.area_code FROM  datag_yancao_historyOrderDetailData y inner join datag_shop s on y.shop_id = s.shop_id inner join datag_shop_tobacco t on s.shop_id = t.shop_id where $CONDITIONS'   -m 1  --target-dir /ycd/input/offline/$year_month/$day/tobacco_order_details_$time_cur --fields-terminated-by '\t' >> $PROJECT_LOG_HOME/offline/$date_cur/sqoop_ycd.log

hdfs dfs -ls /ycd/input/offline/$year_month/$day/tobacco_order_details_$time_cur
...
hdfs dfs -ls /ycd/output/offline/$year_month/$day/$time_cur
