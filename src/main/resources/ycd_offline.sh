...
# 每个店铺需要计算几个月份的数据。
$SQOOP_HOME/bin/sqoop import --connect "jdbc:mysql://$mysql_jdbc?useUnicode=true&characterEncoding=utf-8&zeroDateTimeBehavior=convertToNull&transformedBitIsBoolean=true" --username $mysql_username --password $mysql_password --query 'SELECT distinct s.shop_src_id,ac.count_month FROM  (select ss.shop_src_id as shop_src_id ,ss.shop_id as shop_id from  datag_shop ss  where shop_src_id is not null ) s inner join  datag_shop_tobacco y  on s.shop_id = y.shop_id inner join datag_tobacco_area_code ac on ac.area_code = y.area_code where $CONDITIONS'   -m 1  --target-dir /ycd/input/offline/$year_month/$day/store_id_calc_months_$time_cur --fields-terminated-by '\t' >> $PROJECT_LOG_HOME/offline/$date_cur/sqoop_ycd.log
